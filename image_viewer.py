...
class MainWindow(QMainWindow):
    def __init__(self, settings, parent=None):
        ...
        self.res = int((500 / (720*3)) * screenGeometry.height())
        print(self.res)
...
split('\\')[-1].split('.')[0], self.tile_list, self.folderPath.split('\\')[-1].split('.')[1], self.tile_size))
    ...
    def onSetRowsActionTriggered(self):
        ...
        screenGeometry = QApplication.primaryScreen().availableGeometry()
        self.res = int((500 / (720 * self.nRows)) * screenGeometry.height())
        if self.res * self.nCols > screenGeometry.width() * 0.9:
            self.res = int(screenGeometry.width() * 0.9 / self.nCols)
        self.maxSize = min(screenGeometry.width(), screenGeometry.height()) // self.nRows
    ...
    def onSetSizeActionTriggered(self):
        ...
        if ok and text != '':
            self.tile_size = int( float(text) * 1000 / int(NM_P))
            self.currPage = 0
            self.pageLabel.setText("Page " + str(self.currPage+1) + "/" + str(self.maxPage+1))
            child = self.gridLayout.takeAt(0)
            while child is not None:
...
split('\\')[-1].split('.')[0], self.tile_list, self.folderPath.split('\\')[-1].split('.')[1], self.tile_size))
    ...
    def onAboutActionTriggered(self):
        msgBox = QMessageBox()
        msgBox.setText("image_viewer_V5\n\nThis application lets you view the predictions on any ndpi image in a neat grid format. Double click on tiles to mark them as false positives")
        msgBox.exec()
